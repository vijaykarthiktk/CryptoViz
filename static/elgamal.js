document.getElementById('generateKeys').addEventListener('click', async () => {
    try {
        ...
        const response = await fetch('/generate-elgamal-keys', {
            method: 'POST',
            headers: {
...
document.getElementById('encryptForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('/asymmetric/elgamal/encrypt', {
            method: 'POST',
            body: formData
        });
        ...
        if (response.ok) {
            ...
            a.href = url;
            a.download = 'elgamal_encrypted_' + document.getElementById('encryptFile').files[0].name;
...
document.getElementById('decryptForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('/asymmetric/elgamal/decrypt', {
            method: 'POST',
            body: formData
        });
        ...
        if (response.ok) {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = 'decrypted_' + file.name.replace('elgamal_encrypted_', '');
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            a.remove();
