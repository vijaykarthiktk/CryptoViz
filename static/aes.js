document.getElementById('generateKey').addEventListener('click', async () => {
    try {
        ...
        const response = await fetch('/generate-aes-key', {
            method: 'POST',
            headers: {
...
document.getElementById('encryptForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('/symmetric/aes/encrypt', {
            method: 'POST',
            body: formData
        });
        ...
        if (response.ok) {
            ...
            a.href = url;
            a.download = 'aes_encrypted_' + document.getElementById('encryptFile').files[0].name;
...
document.getElementById('decryptForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('/symmetric/aes/decrypt', {
            method: 'POST',
            body: formData
        });
        ...
        if (response.ok) {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = 'decrypted_' + file.name.replace('aes_encrypted_', '');
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            a.remove();
